...
// Build batch coupling query (filtered by module names)
export function buildBatchCouplingQuery(snapshotId) {
  return function(modulesStr) {
    const modules = splitModules(modulesStr);
    if (modules.length === 0) {
      return `SELECT module, name, external_call_count, external_caller_count, total_coupling, coupling_intensity FROM declaration_coupling WHERE 1=0`;
    }
    const inClause = buildInClause(modules);
    return `
      SELECT module, name, external_call_count, external_caller_count, total_coupling, coupling_intensity
...
// =============================================================================
// Declaration coupling metrics builder
// =============================================================================

export function buildDeclarationCouplingJson(rows) {
  // Build map: { "Module.name": { externalCalls, externalCallers, totalCoupling, intensity } }
  const coupling = {};
  ...
  for (const row of rows) {
    ...
    coupling[key] = {
      ...
      externalCallerCount: row.external_caller_count,
      totalCoupling: row.total_coupling,
      couplingIntensity: row.coupling_intensity
    };
  }

  return JSON.stringify({ declarations: coupling });
}

// =============================================================================
// Module coupling metrics builder
// =============================================================================

export function buildModuleCouplingJson(rows) {
  // Build map: { "ModuleName": { efferent, afferent, instability } }
  const coupling = {};

  for (const row of rows) {
    coupling[row.module] = {
      efferentCoupling: row.efferent_coupling,
      afferentCoupling: row.afferent_coupling,
      instability: row.instability
    };
  }

  return JSON.stringify({ modules: coupling });
}
