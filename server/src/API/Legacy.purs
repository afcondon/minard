...
-- =============================================================================
-- /api/batch-coupling/:modules (comma-separated)
-- =============================================================================

-- | Returns coupling metrics for multiple modules at once
-- | Input: comma-separated module names like "Module.A,Module.B,Module.C"
-- | Output: { "declarations": { "Module.A.fn": {...}, "Module.B.fn": {...} } }
batchCouplingJson :: Database -> Int -> String -> Aff Response
batchCouplingJson db snapshotId modulesStr = do
  let query = buildBatchCouplingQuery snapshotId modulesStr
  rows <- queryAll db query
  let json = buildDeclarationCouplingJson rows
  ok' jsonHeaders json

foreign import buildBatchCouplingQuery :: Int -> String -> String
...
-- =============================================================================
-- /api/declaration-coupling
-- =============================================================================

-- | Returns declaration coupling metrics: { "Module.name": { externalCalls, externalCallers, ... } }
declarationCouplingJson :: Database -> Int -> Aff Response
declarationCouplingJson db snapshotId = do
  rows <- queryAllParams db """
    SELECT
      module,
      name,
      external_call_count,
      external_caller_count,
      total_coupling,
      coupling_intensity
    FROM declaration_coupling
    WHERE snapshot_id = ?
...
foreign import buildDeclarationCouplingJson :: Array Foreign -> String

-- =============================================================================
-- /api/module-coupling
-- =============================================================================

-- | Returns module coupling metrics: { "ModuleName": { efferent, afferent, instability } }
moduleCouplingJson :: Database -> Int -> Aff Response
moduleCouplingJson db snapshotId = do
  rows <- queryAllParams db """
    SELECT
      module,
      efferent_coupling,
  ...
  """ [unsafeToForeign snapshotId]
  let json = buildModuleCouplingJson rows
  ok' jsonHeaders json

foreign import buildModuleCouplingJson :: Array Foreign -> String
