...
type Input =
  ...
  , showDependencyLinks :: Boolean   -- When true, show cross-module call links
  }
...
handleAction = case _ of
  ...
  Receive input -> do
    ...
    let packageChanged = input.packageName /= lastInput.packageName
        ...
        depLinksChanged = input.showDependencyLinks /= lastInput.showDependencyLinks

    -- Update lastInput for next comparison
    H.modify_ _ { lastInput = input }

    when (packageChanged || modulesChanged || declarationsChanged || callsChanged || colorModeChanged || gitStatusChanged || reachabilityChanged || peekChanged || clusterChanged || purityChanged || purityPeekChanged || complexityChanged || complexityPeekChanged || changeFreqChanged || coChangeClusterChanged || sizeByFreqChanged || depLinksChanged) do
...
renderTreemap input = do
  ...
  liftEffect $ ModuleTreemapEnriched.render
    ...
    , showDependencyLinks: input.showDependencyLinks
    }
    pkgModules
    pkgImports
    input.declarations
